...
5. **角色特色**：${character1}和${character2}要有各自的语言特点和观点
6. **新闻结合**：充分利用提供的新闻内容，引用具体事实和数据
7. **对话自然**：语言要自然流畅，符合${dialogueTypeDetails.name}的特点
8. **观点多元**：展现不同角度的思考和讨论
9. **结构完整**：对话要有开头、发展、高潮和总结
10. **中文口语化特点**：
    - 使用日常中文用语和表达方式
    - 适当使用中文语气词和感叹词（如"啊"、"呢"、"吧"、"哦"等）
    - 避免过于正式的学术语言
    - 保持对话的自然节奏和语调
    - 使用中文的思维方式和表达习惯
...
现在请开始生成对话内容：`;
      ...
      const response = await axios.post(llmConfig.apiUrl, {
        ...
        messages: [
          {
            role: "system",
            content: `你是一个专业的对话生成助手，擅长根据新闻内容生成高质量的${dialogueTypeDetails.name}对话。你的对话总是内容丰富、观点深刻、逻辑清晰。你必须严格按照JSON格式输出，不添加任何解释或额外文字。重要提示：对话内容必须使用中文，除了专业的技术词汇（如API、AI、VR、AR、5G、区块链等）和外国的公司名称（如Google、Microsoft、Apple、Meta等）外，其他内容一律使用中文表达。`
          },
          {
            role: "user",
            content: prompt
